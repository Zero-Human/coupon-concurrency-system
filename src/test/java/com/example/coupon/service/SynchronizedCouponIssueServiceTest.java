...
@SpringBootTest
class SynchronizedCouponIssueServiceTest {
    ...
    void setUp() {
        ...
        // 재고 100개짜리 쿠폰 생성
        Coupon coupon = new Coupon("Synchronized 테스트", 100); // 생성자는 실제 코드에 맞게 수정
    ...
    void 동시에_요청해도_재고_이상으로_발급되지_않는다() throws Exception {
        ...
        for (int i = 0; i < threadCount; i++) {
            ...
            executorService.submit(() -> {
                try {
                    synchronizedCouponIssueService.issue(couponId, userId);
                } catch (Exception e) {
                    // 재고 소진 등 예외는 무시
                } finally {
                    latch.countDown();
